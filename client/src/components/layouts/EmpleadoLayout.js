...
const EmpleadoLayout = () => {
  ...
  const menuItems = [
    ...
    { path: '/empleado/horarios', label: 'Horarios', icon: '📅' },
  ...
  const isActive = (path) => {
    if (path === '/empleado') {
      return location.pathname === '/empleado' || location.pathname === '/empleado/';
    }
      ...
      >
        <div className="h-full flex flex-col overflow-y-auto">
          <div className="p-6 bg-green-600 text-white text-center">
            ...
            <h2 className="text-xl font-bold mb-2">{tituloEmpresa}</h2>
            <p className="text-sm opacity-90">{user?.nombre} {user?.apellido}</p>
            <p className="text-xs opacity-75 capitalize">Empleado</p>
          </div>
